...
const OceanGame: React.FC<OceanGameProps> = ({ level, setLevel, onHome }) => {
  const {
    program,
    gameState,
    activeBlockId,
    ...
    resetLevel,
    runProgram
  } = useGameLogic('OCEAN', oceanTheme.missions, level, () => {});

  const cellSize = Math.min(BASE_CELL_SIZE, (window.innerWidth - 40) / gameState.gridSize);
  const totalItems = oceanTheme.missions[level].items.length;

  return (
    <div className={`fixed inset-0 flex flex-col ${oceanTheme.bg} overflow-hidden safe-area-inset`}>
      <GameHeader onHome={onHome} emoji={oceanTheme.emoji} collectedItems={gameState.collectedItems} totalItems={totalItems} level={level} themeKey="OCEAN" />
      <div className="h-[48%] w-full relative flex items-center justify-center p-2 border-b-2 border-white/50 overflow-hidden shrink-0 bg-white/30 shadow-inner">
        ...
        <GameGrid gameState={gameState} cellSize={cellSize} theme={oceanTheme} />
        <GameOverlay isVictory={gameState.items.length === 0 && !gameState.isExecuting && !showFailOverlay && gameState.collectedItems > 0} showFail={showFailOverlay && !gameState.isExecuting} onNext={() => level < oceanTheme.missions.length - 1 ? setLevel(l => l + 1) : onHome()} onFix={() => resetLevel(false)} onClear={() => resetLevel(true)} />
      </div>
