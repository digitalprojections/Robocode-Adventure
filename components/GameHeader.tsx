...
const GameHeader: React.FC<GameHeaderProps> = ({ onHome, emoji, collectedItems, totalItems, level, themeKey }) => {
  return (
    <header className="bg-white px-4 py-2 flex justify-between items-center shadow-sm z-30 shrink-0">
      ...
      <div className="flex gap-2">
          ...
          <div className="bg-amber-100 px-2 py-1 rounded-lg border border-amber-200 text-xs font-bold text-amber-700">L{level + 1}</div>
      </div>
    </header>
  );
};
