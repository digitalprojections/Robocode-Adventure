...
export const useGameLogic = (themeKey: GameTheme, currentMissionSet: any[], level: number, onComplete: () => void) => {
  ...
  const [gameState, setGameState] = useState<GameState>({
    ...
    obstacles: currentMissionSet[level].obstacles ? [...currentMissionSet[level].obstacles] : [],
  ...
  useEffect(() => {
    ...
    setGameState({
      ...
      obstacles: currentLvl.obstacles ? [...currentLvl.obstacles] : [],
  ...
  const executeAction = async (block: CodeBlock, sharedState: { robots: RobotState[], items: Position[], obstacles: Position[], count: number, gridSize: number }): Promise<boolean> => {
      ...
      for (const robot of sharedState.robots) {
        ...
        switch (block.type) {
          case BlockType.MOVE_FORWARD: {
            ...
            const isObstacle = sharedState.obstacles.some(obs => obs.x === nextPos.x && obs.y === nextPos.y);

            if (nextPos.x >= 0 && nextPos.x < sharedState.gridSize && nextPos.y >= 0 && nextPos.y < sharedState.gridSize && !isObstacle) {
              robot.pos = nextPos;
              playSound('move');
            } else {
              playSound('wall');
